...
if not BOT_TOKEN or " " in BOT_TOKEN:
    raise SystemExit("❌ BOT_TOKEN не знайдено або містить пробіли. Додай у Railway → Variables.")
...
try:
    ...
    ADMIN_USER_ID = None
    print("⚠️ ADMIN_USER_ID не задано. Напиши /id боту і додай значення у Railway → Variables.")

bot = Bot(BOT_TOKEN, default=DefaultBotProperties(parse_mode="HTML"))
dp = Dispatcher()

# Зв’язування: message_id у чаті адміна -> user_id користувача
ROUTE: dict[int, int] = {}

# ========= Контент / меню (за бажанням залишив твоє) =========
WELCOME_TEXT = (
...
async def from_user(m: Message):
    # ігноруємо повідомлення від самого адміна у приваті
    ...
    header = (
        ...
        "✍️ Відповідай <b>реплаєм на копію</b> нижче — я перешлю користувачу."
    )
    # 1) шапка
    await bot.send_message(ADMIN_USER_ID, header, disable_web_page_preview=True)
    # 2) копія повідомлення (збережуться медіа)
    copied = await m.copy_to(ADMIN_USER_ID)
    # 3) запам’ятовуємо, що відповідь на це повідомлення має піти користувачу
    ROUTE[copied.message_id] = u.id

# ========= Адмін → Користувач (реплай на копію) =========
...
async def from_admin_reply(m: Message):
    ...
    if not user_id:
        await m.reply("ℹ️ Відповідай саме на <b>копію</b> повідомлення користувача (не на шапку).")
        return
